...
export function renderFigmaPanelUI({ sidebar, getSelectedEl, onStyleEdit }) {
  ...
  // Load the HTML UI from Figmapanelnew.html
...
async function loadFigmaPanelHTML(sidebar, { getSelectedEl, onStyleEdit }) {
  try {
    ...
    const response = await fetch(chrome.runtime.getURL('Figmapanelnew.html'));
    ...
    console.error('Error loading Figmapanelnew HTML:', error);
  ...
  const paddingInputs = {
    left: container.querySelector('[data-layer="150"]'),
    right: container.querySelectorAll('[data-layer="150"]')[1], // Second occurrence
    top: container.querySelector('[data-layer="120"]'),
    bottom: container.querySelectorAll('[data-layer="120"]')[1] // Second occurrence
  };
  ...
  Object.entries(paddingInputs).forEach(([side, input]) => {
    if (input) {
      ...
      input.addEventListener('input', (e) => {
        ...
        if (el) {
          const value = e.target.textContent.replace(/[^\d]/g, '');
          onStyleEdit(`padding${side.charAt(0).toUpperCase() + side.slice(1)}`, value + 'px');
        }
      });
    }
  });
...
function updateUIFromSelectedElement(container, element) {
  ...
  const paddingInputs = {
    left: container.querySelector('[data-layer="150"]'),
    right: container.querySelectorAll('[data-layer="150"]')[1],
    top: container.querySelector('[data-layer="120"]'),
    bottom: container.querySelectorAll('[data-layer="120"]')[1]
  };
  ...
  Object.entries(paddingInputs).forEach(([side, input]) => {
    if (input) {
      const padding = style[`padding${side.charAt(0).toUpperCase() + side.slice(1)}`] ? 
        parseInt(style[`padding${side.charAt(0).toUpperCase() + side.slice(1)}`]) : '';
      input.textContent = padding !== '' ? padding : (side === 'left' || side === 'right' ? '150' : '120');
    }
  });
